...
class GenerateDNQueries(nn.Module):
    ...
    def generate_query_masks(self, max_gt_num_per_image):
        noised_query_nums = max_gt_num_per_image * self.denoising_groups
        tgt_size = noised_query_nums + self.num_queries
        attn_mask = torch.zeros(tgt_size, tgt_size, device='cuda', dtype=torch.bool)
    ...
    def forward(self, max_gt_num_per_image):
        """
        ...
        attn_mask = self.generate_query_masks(max_gt_num_per_image)
...
class GenerateCDNQueries(GenerateDNQueries):
    ...
    def forward(self, max_gt_num_per_image):
        ...
        # generate attention masks for transformer layers
        attn_mask = self.generate_query_masks(2 * max_gt_num_per_image)
