...
label_map = {
    0: 'Inf', 1: 'LF', 2: 'KF'
    # 0: 'Inflorescence', 1: 'LateralFlower', 2: 'KingFlower'
...
def draw(image, labels, boxes, scores, thrh=0.45, file_path=''):
    ...
    for j, box in enumerate(boxes):
        ...
        draw.text((box[0] + 2, box[1] - text_height - 10), text, fill="black", font=font)

        line= str(category) + ' ' + str(box[0]) + ' ' + str(box[1]) + ' ' + str(box[2]) + ' ' + str(box[3])
        texts.append(line)

    txtpath = './visout/' + os.path.basename(file_path).replace('jpg', 'txt')
...
def process_dataset(model, dataset_path, output_path, thrh=0.5, size=(640, 640), vit_backbone=False):
    ...
    for idx, file_path in enumerate(image_paths):
        ...
        # 绘制结果
        vis_image = draw(im_pil.copy(), labels, boxes, scores, thrh, file_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='./datasets/images/val')
